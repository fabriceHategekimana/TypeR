...
pub fn parse_double(s: &str) -> IResult<&str, Value> {
    let res = tuple((
               opt(tag("-")),
               digit1,
               floating_point
    ...
    match res {
        Ok((s, (Some(m), v, r))) => 
            Ok((s, Value::new(&format!("{}{}{}", m, v, r), Type::Scalar(BaseType::Double)))),
        Ok((s, (None, v, r))) => 
            Ok((s, Value::new(&format!("{}{}", v, r), Type::Scalar(BaseType::Double)))),
        Err(e) => Err(e)
    }
}
